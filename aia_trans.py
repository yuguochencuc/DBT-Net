...
from aia_inter_new import AIA_Transformer, AIA_Transformer_merge, AHAM
...
class dual_aia_trans_ri(nn.Module):
    def __init__(self):
        super(dual_aia_trans_ri, self).__init__()
        self.en_ri = dense_encoder()
        self.en_ri_res = dense_encoder()
        #self.en_mag = dense_encoder_mag()
        self.aia_trans_merge = AIA_Transformer_merge(128, 64, num_layers=4)
        self.aham = AHAM(input_channel=64)
        self.aham_res = AHAM(input_channel=64)

        self.de1 = dense_decoder()
        self.de2 = dense_decoder()
        self.de1_res = dense_decoder()
        self.de2_res = dense_decoder()
        #self.de_mag_mask = dense_decoder_masking()
    ...
    def forward(self, x):
        ...
        x_ri = self.en_ri(x) #BCTF
        x_ri_res = self.en_ri_res(x) #BCTF
        x_last_ri, x_outputlist_ri, x_last_ri_res, x_outputlist_ri_res = self.aia_trans_merge(x_ri, x_ri_res)  # BCTF, #BCTFG


        x_ri = self.aham(x_outputlist_ri) #BCT
        x_ri_res = self.aham_res(x_outputlist_ri_res)  # BCT

        # real and imag decode
        x_real = self.de1(x_ri)
        x_imag = self.de2(x_ri)
        x_real_res = self.de1_res(x_ri_res)
        x_imag_res = self.de2_res(x_ri_res)
        x_real = x_real.squeeze(dim = 1)
        x_imag = x_imag.squeeze(dim=1)
        x_real_res = x_real_res.squeeze(dim = 1)
        x_imag_res = x_imag_res.squeeze(dim=1)
        # magnitude and ri components interaction

        #x_mag_out=x_mag_mask * x_mag_ori
        x_r_out,x_i_out = (x_real_res + x_real), (x_imag_res + x_imag)
...
class dual_aia_trans_merge_crm_atfa(nn.Module):
    def __init__(self):
        super(dual_aia_trans_merge_crm_atfa, self).__init__()
        self.en_ri = dense_encoder()
        self.en_mag = dense_encoder_mag()
        self.aia_trans_merge = AIA_Transformer_merge(128, 64, num_layers=4)
        #self.aham = AHAM(input_channel=64)
    ...
    def forward(self, x):
        ...
        #x_ri = self.aham(x_outputlist_ri) #BCT
        #x_mag_en = self.aham_mag(x_outputlist_mag)  # BCTF
        x_mag_mask = self.de_mag_mask(x_last_mag)
        x_mag_mask = x_mag_mask.squeeze(dim=1)

        # real and imag decode
        x_real = self.de1(x_last_ri)
        x_imag = self.de2(x_last_ri)
        x_real = x_real.squeeze(dim = 1)
        x_imag = x_imag.squeeze(dim=1)
        # magnitude and ri components interaction
